...
//
//
//    EntityValue customer = ef.one()
//
//    if (customer == null) {
//        throw new IllegalStateException("Customer not found for email: " + emailAddress);
//    }
for(customerIdV in partyIdL){

    EntityFind ef1 = ec.entity.find("assignment.contact.PartyContactMech").distinct(true)
    ...
    for (id in contactMechIdList) {
        EntityFind ef2 = ec.entity.find("assignment.contact.PartyContactMech").distinct(true)
        OrderedList = [
        ...
        if (ev2.contactMechPurposeId == "PostalPrimary") {
            ...
            if (toName || attnName || address1 || address2 || directions || city || postalCode) {
                ...
                def postalAddressEntity = ec.entity.makeValue("assignment.contact.PostalAddress")
                fields = [
                        "contactMechId": contactMech.contactMechId,
...
//            context.message="aagya"
            if (coutryCode || areaCode || mobileNumber) {
                ...
                def TelecomNumber = ec.entity.makeValue("assignment.contact.TelecomNumber")
                fields = [
                        "contactMechId": contactMech.contactMechId,
...
//            context.telecomId = id
        }

//    context.telecomId=ev2.contactNumbertMechPurposeId
    }
